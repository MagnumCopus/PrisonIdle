...
function newConnection(socket) {
	...
	socket.on('movement', function (movement) {
		...
		if (player.inputs == null) player.inputs = [];
	    player.inputs.push(movement);
	});

	// Physics Loop
	var lastPhysicsLoop = new Date().getTime();
	var loopCount = 0;
	setInterval(function() {
		var dTime = (new Date().getTime() - lastPhysicsLoop) / 1000;
		if (loopCount++ > 10) {
			console.log((1 / dTime).toFixed(0));
			loopCount = 0;
		}
		for (var id in players) {
			var player = players[id];
			if (player.inputs == null) player.inputs = [];
			player.inputs.forEach(function(movement) {
				if (movement.left) {
		          player.x -= 240 * dTime;
		        }
		        if (movement.up) {
		...
		lastPhysicsLoop = new Date().getTime();
	}, 1000 / 66);
